...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const apiKey = setting.apiKey // contact me on whatsapp wa.me/5527997460219
			const { text, extendedText, contact, locan, liveLocation, image, video, sticker, document, audio, product } = MessageType
			...
			function addMetadata(packname, author) {	
				if (!packname) packname = 'RedePoseidon'; if (!author) author = 'Elbielzinho';	
			...
			switch(command) {
				...
				case 'ep':
					if (args.length < 1) {
						return reply('Escolha o tema, fih, 1 - 216')
					} else if (args[0].toLowerCase() === 'list') {
						teks = await fetchText('https://mhankbarbar.moe/api/ephoto/listtheme')
						teks = teks.replace(/<br>/g, '\n')
						return reply(teks)
					} else if (args.length < 2) {
						return reply('Coloca o texto também, né?')
					}
					reply(mess.wait)
				...
				case 'sticker':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								console.log('Finish')
								exec(`webpmux -set exif ${addMetadata('Elbielzinho', authorname)} ${ran} -o ${ran}`, async (error) => {
						...
						await ffmpeg(`./${media}`)
							...
							.addOutputOptions([`-vcodec`,`libwebp`,`20,iw)':min'(320,ih)':force_original_aspect_ratio=decrease,fps=15, pad=320:320:-1:-1:color=white@0.0, split [a][b]; [a] palettegen=reserve_transparent=on:transparency_color=ffffff [p]; [b][p] paletteuse`])
							.toFormat('webp')
							.save(ran)
				...
				case 'gtts':
					...
					if (args.length < 2) return client.sendMessage(from, '💬Cadê o texto meu fih💬 {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					dtt.length > 600
					? reply(💭'Tem muito texto ai, tira um pouco💭')
				...
				case 'setprefix':
					...
					fs.writeFileSync('./src/settings.json', JSON.stringify(setting, null, '\ply(`O prefixo foi alterado com sucesso para : ${prefix}`)
					break
				/*case 'loli':
				...
				case 'nsfwloli':
					...
					loli.getNSFWLoli(async (err, res) => {
						...
						buffer = await getBuffer(res.url)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Não faço ingredientes para vc'})
					})
					break
				case 'hilih':
				...
				case 'clearall':
					...
					reply('✅ Limpado com sucesso o chat todo :)✅')
					break
				case 'bc':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Promova o sucesso @${mentioned[0].split('@')[0]} Como administrador do grupo!😘`, mentioned, true)
                ...
                case 'leave':
                    ...
                    break
				case 'toimg':
					if (!isQuotedSticker) return reply('😄 Oiá a foto 😄')
					reply(mess.wait)
				...
				case 'simi':
					...
					//if (anu.error) return reply('Deu erro :(')
					reply(anu)
					break
				case 'simih':
					if (!isGroup) return reply(mess.only.group)
					...
					if (Number(args[0]) === 1) {
						...
						reply(😍'Ativar o modo simi com sucesso neste grupo😍')
					} else if (Number(args[0]) === 0) {
						samih.splice(from, 1)
						fs.writeFileSync('./src/simi.json', JSON.stringify(samih))
						reply('😭Desativando com sucesso o modo simi neste grupo😭')
					} else {
